...
export default function NotifyForm({ vertical }: { vertical: string }) {
  ...
  function onSubmit(e: React.FormEvent) {
    ...
    startTransition(async () => {
      const res = await submitVerticalInterest({ vertical, email, phoneCc, phone });
      ...
      {!done && (
        <form onSubmit={onSubmit} className="mt-5">
          <div className="flex flex-col gap-3">
            ...
            {/* WhatsApp cue — this is the number we'll reach out on */}
            <p className="flex items-center gap-1.5 text-xs text-[color:var(--ink-soft)] leading-relaxed">
              <svg
                aria-hidden
                viewBox="0 0 24 24"
              ...
              We&rsquo;ll send launch updates on WhatsApp.
            </p>
